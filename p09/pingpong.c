...
task_t *InitTask, *AtualTask, **filaDeProntas, **filaSleep;
task_t MainTask, Dispatcher, INIT;
int contadorTarefas;
int ticksTarefa=20;
int relogio=0;
...
void pingpong_init(){
    ...
    filaDeProntas=0;
    task_create(&MainTask, main, "Main Task");
    //inicia o Timer
...
int task_create (task_t *task, void (*start_routine)(void *),void *arg){
    ...
    //definição dos tempos de inicio e processador
...
void task_exit(int exit_code){
    ...
    //    userTasks--;}
    AtualTask->exitCode=exit_code;
    printf("Task %d exit: execution time %d ms, processor time %d ms, %d activations\n", AtualTask->tid, (systime()-AtualTask->horaInicio), AtualTask->horaUso, AtualTask->ativation);
...
void dispatcher_body () // dispatcher é uma tarefa
{
    ...
    while ( userTasks > 0 )
    {
        ...
        if (next)
        {
            ...
            next->horaUso += systime()-tempoInicio;
            next->ativation++;
